...
export default class NveTag extends LitElement {
  ...
  /** Ekstra tekst */
  @property({ type: String, reflect: false, attribute: 'extra-text' }) extraText = '';
  /** Viser dot-ikon i prefix */
  @property({ type: Boolean, reflect: false }) dot: boolean = false;
  /** Viser lukke-knapp */
  @property({ type: Boolean, reflect: false, attribute: 'closeable' }) closeable: boolean = false;
  /** aria-label på lukke-knapp */
  @property({ type: String, reflect: false, attribute: 'close-aria-label' }) closeAriaLabel: string = 'Lukk';
  ...
  render() {
    return html`
      ...
      >
        ...
        >
        <slot part="text"></slot>
        <slot name="extra" part="extra"
          >${this.extraText ? html`<span class="extra-text">${this.extraText}</span>` : nothing}</slot
        >
        ${this.closeable
          ? html`<button
              part="close"
