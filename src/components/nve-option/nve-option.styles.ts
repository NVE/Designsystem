import { css } from 'lit';

export default css`
  :host {
    font: var(--typography-body-small);
    --sl-font-sans: var(--font-families-source-sans-pro);
    color: var(--color-neutrals-foreground-primary, #0d0d0e);
  }

  .option--hover:not(.option--current):not(.option--disabled) {
    background-color: var(--color-neutrals-background-primary-contrast, #f7f7f8);
    color: var(--color-neutrals-foreground-primary, #0d0d0e);
  }
  .option--selected:not(.option--disabled) {
    background-color: var(--color-neutrals-background-secondary, #e4e5e7);
    color: var(--color-neutrals-foreground-primary, #0d0d0e);
  }

  :host([hide-checkmark])::part(checked-icon) {
    display: none !important;
  }

  .option--current,
  .option--current.option--disabled {
    color: var(--color-neutrals-foreground-primary, #0d0d0e);
    background: var(--color-neutrals-background-primary-contrast, #eff8fc);
  }

  :host(:focus-visible) .option {
    box-shadow: none;
    background-color: var(--color-neutrals-background-secondary, #e4e5e7);
  }
  :host(:focus-visible) .option.option--disabled {
    opacity: 0.5;
  }
  /* hvit hover når filled */
  :host([filled]) .option--hover:not(.option--current):not(.option--disabled):hover {
    background-color: var(--color-neutrals-background-primary, #fff);
  }

  :host([filled]) .option {
    color: var(--color-neutrals-foreground-primary, #0d0d0e);
  }
`;
